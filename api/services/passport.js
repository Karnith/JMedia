...
passport.connect = function (req, query, profile, next) {
    ...
    sails.log.debug('Setting auth provider');
    ...
    if (profile.hasOwnProperty('emails')) {
      ...
      sails.log.debug('Grabbing first email in list: '+user.email);
    }
    // If the profile object contains a username, add it to the user.
    if (profile.hasOwnProperty('username')) {
      user.username = profile.username;
      sails.log.debug('Profile conatins user name: '+user.username);
    }
    // If the profile object doesn't contain a display name
    ...
    Passport.findOne({
        ...
        if (!req.user) {
            ...
            if (!passport) {
                sails.log.debug('Creating new user');
                console.log(user);
                User.create(user).exec(function (err, user) {
                    ...
                    sails.log.debug('Creating auth for new user');
                    waterlock.engine.attachAuthToUser({email : user.email}, user, function (err) {
                        if (err) {
                            sails.log.debug(err);
                            return done(err);
                        }
                        query.user = user.id;
                        sails.log.debug('Creating passport');
            ...
            // Action:   Get the user associated with the passport.
                ...
                // If the tokens have changed since the last session, update them
                  sails.log.debug('Updating Expired Token');
                if (query.hasOwnProperty('tokens') && query.tokens !== passport.tokens) {
                  passport.tokens = query.tokens;
                }

                // Save any updates to the Passport before moving on
                sails.log.debug('Saving updates to passport');
                passport.save(function (err, passport) {
                    if (err) return next(err);

                    // Fetch the user associated with the Passport
                    sails.log.debug('Finding user to associate with passport');
                    User.findOne(passport.user.id).populate('auth').exec(next);
                });
            }
        }
        else {
            ...
            if (!passport) {
                ...
                sails.log.debug('Creating passport');
                Passport.create(query).populate('auth').exec(function (err, passport) {
                    // If a passport wasn't created, bail out
                    if (err) return next(err);
            ...
            // Action:   Simply pass along the already established session.
                ...
                sails.log.debug('Passing already establish session');
                next(null, req.user);
            }
        }
    });
...
passport.endpoint = function (req, res) {
  ...
  if (!strategies.hasOwnProperty(provider)) {
      sails.log.debug('Sending to login');
    return res.redirect('/login');
  }

  // Attach scope if it has been set in the config
  if (strategies[provider].hasOwnProperty('scope')) {
      sails.log.debug('Attaching scope');
  ...
  //     /auth/:provider/callback
    sails.log.debug('Sending user to provider');
...
passport.loadStrategies = function () {
  ...
  Object.keys(strategies).forEach(function (key) {
    ...
    if (key === 'local') {
      ...
      if (strategies.local) {
          sails.log.debug('Loading local passport strategy');
      ...
      if (!callback) {
          sails.log.debug('compiling callback');
      ...
      switch (protocol) {
        case 'oauth':
        case 'oauth2':
                sails.log.debug('oauth2 being used');
                options.callbackURL = url.resolve(baseUrl, callback);
        break;

        case 'openid':
                sails.log.debug('Openid being used');
      ...
      _.extend(options, strategies[key].options);
        ...
        sails.log.debug('Creating new strategy for use');
